...
export async function main(ns) {
	while (true) {
		...
		DispatchHacked(ns, shouldShare);
		DispatchPurchased(ns, shouldShare);
		await ns.sleep(500);
	}
}
...
function DispatchPurchased(ns, shouldShare) {
	for (var x = 0; x < purchasedServers.length; x++) {
		var serverName = purchasedServers[x];
		ns.print("-------" + serverName + "-------");
		var endedScripts = GetServerEndedScripts(ns, serverName);
		var numberServers = endedScripts.length;
...
function DispatchHacked(ns, shouldShare) {
	for (var x = 0; x < hacked.length; x++) {
		var serverName = hacked[x];
		ns.print("-------" + serverName + "-------");
		var endedScripts = GetServerEndedScripts(ns, serverName);
		var numberServers = endedScripts.length;
...
function GetServerEndedScripts(ns, serverName) {
	var endedScripts = []
	for (var x = 0; x < hacked.length; x++) {
		if (!TargetServerScriptsRunning(ns, serverName, hacked[x])) {
			endedScripts.push(hacked[x]);
		}
	}
	return endedScripts;
}
